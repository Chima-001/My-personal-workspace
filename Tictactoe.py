...
def check_winner():
    ...
    for row in range(3):
        if (board[row][0]["text"] == board[row][1]["text"] == board[row][2]["text"] and board[row][0]["text"] != ""):
            ...
            for column in range(3):
                board[row][column].config(foreground=color_yellow, background = color_l_gray)
            game_over = True
            return
    ...
    if (board[0][0]["text"] == board[1][1]["text"] == board[2][2]["text"] and board[0][0]["text"] != ""):
        ...
        for i in range(3):
            board[i][i].config(foreground=color_yellow, background=color_l_gray)
        game_over = True
        return
    ...
    if (board[0][2]["text"] == board[1][1]["text"] == board[2][0]["text"] and board[0][2]["text"] != ""):
        ...
        board[0][2].config(foreground=color_yellow, background=color_l_gray)
        board[1][1].config(foreground=color_yellow, background=color_l_gray)
        board[2][0].config(foreground=color_yellow, background=color_l_gray)
        game_over = True
        return
...
button = tkinter.Button(frame, text="Restart", font=("Consolas", 20), background=color_gray, foreground="green", command=new_game)
